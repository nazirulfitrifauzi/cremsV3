...
<main class="flex-1 relative z-0 overflow-y-auto py-6 focus:outline-none" tabindex="0" x-data="" x-init="$el.focus()">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="py-4">
            <form class="bg-white shadow overflow-hidden px-8 py-4 sm:rounded-lg" action="{{ route('profile.update', auth()->user()->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                        ...
                        <div class="mt-4 grid grid-cols-1 row-gap-6 col-gap-4 sm:grid-cols-6 border-t border-gray-200 pt-4">
                            <div class="sm:col-span-2 sm:row-span-2">
                                ...
                                <div class="mt-2 flex items-center">
                                    ...
                                    <span id="avatar_default" class="{{ (auth()->user()->avatar !== NULL) ? 'hidden' : '' }} h-12 2-12 sm:h-32 sm:w-32 rounded-full overflow-hidden bg-gray-500 {{ ($errors->get('avatar') ? 'border-4 border-red-300 ' : '') }}">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="mt-1 rounded-md shadow-sm">
                                    <select id="state" name="state" 
                                        ...
                                        <option disabled {{ (isset($profile->state)) ? '' : 'selected' }}>- Choose State -</option>
                                        <option value="johor" {{ (!$profile) ? '': (($profile->state == 'johor') ? 'selected' : '') }}>Johor</option>
                                        <option value="kedah" {{ (!$profile) ? '': (($profile->state == 'kedah') ? 'selected' : '') }}>Kedah</option>
                                        <option value="kelantan" {{ (!$profile) ? '': (($profile->state == 'kelantan') ? 'selected' : '') }}>Kelantan</option>
                                        <option value="melaka" {{ (!$profile) ? '': (($profile->state == 'melaka') ? 'selected' : '') }}>Melaka</option>
                                        <option value="negeri sembilan" {{ (!$profile) ? '': (($profile->state == 'negeri sembilan') ? 'selected' : '') }}>Negeri Sembilan</option>
                                        <option value="pahang" {{ (!$profile) ? '': (($profile->state == 'pahang') ? 'selected' : '') }}>Pahang</option>
                                        <option value="perak" {{ (!$profile) ? '': (($profile->state == 'perak') ? 'selected' : '') }}>Perak</option>
                                        <option value="perlis" {{ (!$profile) ? '': (($profile->state == 'perlis') ? 'selected' : '') }}>Perlis</option>
                                        <option value="pulau pinang" {{ (!$profile) ? '': (($profile->state == 'pulau pinang') ? 'selected' : '') }}>Pulau Pinang</option>
                                        <option value="sabah" {{ (!$profile) ? '': (($profile->state == 'sabah') ? 'selected' : '') }}>Sabah</option>
                                        <option value="sarawak" {{ (!$profile) ? '': (($profile->state == 'sarawak') ? 'selected' : '') }}>Sarawak</option>
                                        <option value="selangor" {{ (!$profile) ? '': (($profile->state == 'selangor') ? 'selected' : '') }}>Selangor</option>
                                        <option value="terengganu" {{ (!$profile) ? '': (($profile->state == 'terengganu') ? 'selected' : '') }}>Terengganu</option>
                                        <option value="wp kuala lumpur" {{ (!$profile) ? '': (($profile->state == 'wp kuala lumpur') ? 'selected' : '') }}>WP Kuala Lumpur</option>
                                    </select>
                                </div>
                                @error('state') 
                                    <p class="text-red-500 text-xs italic mt-4"> 
